...
router.post('/', requireRole('admin'), validate(createSchema), asyncHandler(async (req: AuthRequest, res) => {
  ...
  await logAudit({ userId: req.user!.userId, action: 'create', entityType: 'tag', entityId: tag.id, details: { name: tag.name } })
...
router.delete('/:id', requireRole('admin'), asyncHandler(async (req: AuthRequest, res) => {
  ...
  await logAudit({ userId: req.user!.userId, action: 'delete', entityType: 'tag', entityId: id })
...
router.post('/document/:documentId', validate(z.object({ tagId: z.number() })), asyncHandler(async (req: AuthRequest, res) => {
  ...
  await logAudit({ userId: req.user!.userId, action: 'add_tag', entityType: 'document', entityId: documentId, details: { tagId: req.body.tagId } })
...
router.delete('/document/:documentId/:tagId', asyncHandler(async (req: AuthRequest, res) => {
  ...
  await logAudit({ userId: req.user!.userId, action: 'remove_tag', entityType: 'document', entityId: documentId, details: { tagId } })
